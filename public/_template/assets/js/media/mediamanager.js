...
//		alert('MediaManager');
	...
	MediaManager.prototype.getSelectedItems = function(returnNotProcessed,
		...
		var items = this.$el.get(0).querySelectorAll(
				'[data-type="media-item"].selected'), result = []
	...
	MediaManager.prototype.init = function() {
		this.itemListElement = this.$el.find('[data-control="item-list"]').get(
				0)
		this.scrollContentElement = this.itemListElement
				.querySelector('.scroll-wrapper')

		if (this.options.bottomToolbar) {
			this.$el.find('[data-control="bottom-toolbar"]')
					.removeClass('hide')

			if (this.options.cropAndInsertButton)
				this.$el.find('[data-popup-command="crop-and-insert"]')
						.removeClass('hide')
		}
//		alert('init');
	...
	MediaManager.prototype.registerHandlers = function() {
		...
		this.$el.on('click.tree-path',
				'ul.tree-path, [data-control="sidebar-labels"]', this
						.proxy(this.onNavigate))

		this.$el.on('click.command', '[data-command]', this
				.proxy(this.onCommandClick))
//alert('registerHandlers');
		// Touch devices use double-tap for the navigation and single tap for
		// selecting.
		// Another option is checkboxes visible only on touch devices, but this
		// approach
		// will require more significant changes in the code for the touch
		// device support.
		if (!Modernizr.touch)
			this.$el.on('click.item', '[data-type="media-item"]', this
					.proxy(this.onItemClick))
		else
			this.$el.on('touchend', '[data-type="media-item"]', this
					.proxy(this.onItemTouch))

		this.$el.on('change', '[data-control="sorting"]', this
				.proxy(this.onSortingChanged))
		this.$el.on('keyup', '[data-control="search"]', this
				.proxy(this.onSearchChanged))
		this.$el.on('mediarefresh', this.proxy(this.refresh))
		this.$el.on('shown.oc.popup', '[data-command="create-folder"]', this
				.proxy(this.onFolderPopupShown))
		this.$el.on('hidden.oc.popup', '[data-command="create-folder"]', this
				.proxy(this.onFolderPopupHidden))
		this.$el.on('shown.oc.popup', '[data-command="move"]', this
				.proxy(this.onMovePopupShown))
		this.$el.on('hidden.oc.popup', '[data-command="move"]', this
				.proxy(this.onMovePopupHidden))
		this.$el.on('keydown', this.proxy(this.onKeyDown))

		if (this.itemListElement)
			this.itemListElement.addEventListener('mousedown', this
					.proxy(this.onListMouseDown))
	}
	...
	MediaManager.prototype.unregisterHandlers = function() {
		...
		if (!Modernizr.touch)
			...
			this.$el.off('touchend', '[data-type="media-item"]', this
					.proxy(this.onItemTouch))

		this.$el.off('change', '[data-control="sorting"]', this
				.proxy(this.onSortingChanged))
		this.$el.off('keyup', '[data-control="search"]', this
				.proxy(this.onSearchChanged))
		this.$el.off('shown.oc.popup', '[data-command="create-folder"]', this
				.proxy(this.onFolderPopupShown))
		this.$el.off('hidden.oc.popup', '[data-command="create-folder"]', this
				.proxy(this.onFolderPopupHidden))
		this.$el.off('shown.oc.popup', '[data-command="move"]', this
				.proxy(this.onMovePopupShown))
		this.$el.off('hidden.oc.popup', '[data-command="move"]', this
				.proxy(this.onMovePopupHidden))
		this.$el.off('keydown', this.proxy(this.onKeyDown))

		if (this.itemListElement) {
			this.itemListElement.removeEventListener('mousedown', this
					.proxy(this.onListMouseDown))
			this.itemListElement.removeEventListener('mousemove', this
					.proxy(this.onListMouseMove))
		}
...
//		alert('removeAttachedControls');
	...
	MediaManager.prototype.selectItem = function(node, expandSelection) {
		if (!expandSelection) {
			...
			// The class attribute is used only for selecting, it's safe to
			// clear it
			for (var i = 0, len = items.length; i < len; i++) {
		...
		if (this.isPreviewSidebarVisible()) {
			...
			this.selectTimer = setTimeout(
					this.proxy(this.updateSidebarPreview), 100)
		}
	...
	MediaManager.prototype.unselectRoot = function() {
		var rootItem = this.$el.get(0).querySelector(
				'[data-type="media-item"][data-root].selected')
	...
	MediaManager.prototype.selectFirstItem = function() {
		var firstItem = this.itemListElement
				.querySelector('[data-type="media-item"]:first-child')
		if (firstItem) {
	...
	MediaManager.prototype.execNavigationRequest = function(handler, data,
			element) {
		...
		this.navigationAjax = element.request(
				this.options.alias + '::' + handler, {
					data : data
				}).always(function() {
			$.oc.stripeLoadIndicator.hide()
		}).done(this.proxy(this.afterNavigate)).always(
				this.proxy(this.releaseNavigationAjax))
	}
	...
	MediaManager.prototype.isPreviewSidebarVisible = function() {
		return !this.$el.find('[data-control="preview-sidebar"]').hasClass(
				'hide')
	}

	MediaManager.prototype.toggleSidebar = function(ev) {
		var isVisible = this.isPreviewSidebarVisible(), $sidebar = this.$el
				.find('[data-control="preview-sidebar"]'), $button = $(ev.target)
	...
	MediaManager.prototype.updateSidebarMediaPreview = function(items) {
		var previewPanel = this.sidebarPreviewElement, previewContainer = previewPanel
				.querySelector('[data-control="media-preview-container"]'), template = ''
		...
		if (items.length == 1 && !items[0].hasAttribute('data-root')) {
			var item = items[0], documentType = item
					.getAttribute('data-document-type')

			switch (documentType) {
			case 'audio':
				template = previewPanel
						.querySelector('[data-control="audio-template"]').innerHTML
				break;
			case 'video':
				template = previewPanel
						.querySelector('[data-control="video-template"]').innerHTML
				break;
			case 'image':
				template = previewPanel
		...
		// No selection
			...
			template = previewPanel
					.querySelector('[data-control="no-selection-template"]').innerHTML
			previewContainer.innerHTML = template
		}
		// Multiple selection
		else {
			template = previewPanel
					.querySelector('[data-control="multi-selection-template"]').innerHTML
			previewContainer.innerHTML = template
		}
	}
	...
	MediaManager.prototype.updateSidebarPreview = function(resetSidebar) {
		if (!this.sidebarPreviewElement)
			this.sidebarPreviewElement = this.$el.get(0).querySelector(
					'[data-control="preview-sidebar"]')

		var items = resetSidebar === undefined ? this.$el.get(0)
				.querySelectorAll('[data-type="media-item"].selected') : [], previewPanel = this.sidebarPreviewElement

		// No items are selected
		if (items.length == 0) {
			this.sidebarPreviewElement.querySelector(
					'[data-control="sidebar-labels"]').setAttribute('class',
					'hide')
		}
		// One item is selected - display the details
			...
			var item = items[0], lastModified = item
					.getAttribute('data-last-modified')

			previewPanel.querySelector('[data-label="size"]').textContent = item
					.getAttribute('data-size')
			previewPanel.querySelector('[data-label="title"]').textContent = item
					.getAttribute('data-title')
			previewPanel.querySelector('[data-label="last-modified"]').textContent = lastModified
			previewPanel.querySelector('[data-label="public-url"]')
					.setAttribute('href', item.getAttribute('data-public-url'))

			if (lastModified)
				previewPanel.querySelector('[data-control="last-modified"]')
						.setAttribute('class', '')
			else
				previewPanel.querySelector('[data-control="last-modified"]')
			...
			if (this.isSearchMode()) {
				previewPanel.querySelector('[data-control="item-folder"]')
						.setAttribute('class', '')
				var folderNode = previewPanel
						.querySelector('[data-label="folder"]')
				folderNode.textContent = item.getAttribute('data-folder')
				folderNode.setAttribute('data-path', item
	...
	MediaManager.prototype.loadSidebarThumbnail = function() {
		...
		this.sidebarThumbnailAjax = this.$form.request(
				this.options.alias + '::onGetSidebarThumbnail', {
					data : data
				}).done(this.proxy(this.replaceSidebarPlaceholder)).always(
				this.proxy(this.releaseSidebarThumbnailAjax))
	}
	...
	MediaManager.prototype.handleThumbnailQueue = function() {
		...
		for (var i = this.activeThumbnailQueueLength; i < maxThumbnailQueueLength
			...
			for (var j = 0; j < maxThumbnailBatchLength
					&& this.thumbnailQueue.length > 0; j++)
	...
	MediaManager.prototype.handleThumbnailBatch = function(batch) {
		...
		var promise = this.$form.request(this.options.alias
	...
	MediaManager.prototype.getRelativePosition = function(element, pageX,
			pageY, startPosition) {
		var absolutePosition = startPosition !== undefined ? startPosition
				: $.oc.foundation.element.absolutePosition(element, true)

		return {
			x : (pageX - absolutePosition.left), // There is no horizontal
													// scroll
	...
	MediaManager.prototype.createSelectionMarker = function() {
		...
		this.scrollContentElement.insertBefore(this.selectionMarker,
				this.scrollContentElement.firstChild)
	}

	MediaManager.prototype.doObjectsCollide = function(aTop, aLeft, aWidth,
			aHeight, bTop, bLeft, bWidth, bHeight) {
		return !(((aTop + aHeight) < (bTop)) || (aTop > (bTop + bHeight))
				|| ((aLeft + aWidth) < bLeft) || (aLeft > (bLeft + bWidth)))
	}
	...
	MediaManager.prototype.initUploader = function() {
		...
		// fallback: implement method that would set a flag that the uploader is
		// not supported by the browser
		}

		this.dropzone = new Dropzone(this.$el.get(0), uploaderOptions)
		this.dropzone.on('addedfile', this.proxy(this.uploadFileAdded))
		this.dropzone.on('totaluploadprogress', this
				.proxy(this.uploadUpdateTotalProgress))
	...
	MediaManager.prototype.uploadUpdateTotalProgress = function(uploadProgress,
			totalBytes, totalBytesSent) {
		this.setUploadProgress(uploadProgress)

		var fileNumberLabel = this.$el.get(0).querySelector(
				'[data-label="file-number-and-progress"]'), messageTemplate = fileNumberLabel
				.getAttribute('data-message-template'), fileNumber = this.dropzone
				.getUploadingFiles().length
		...
		fileNumberLabel.innerHTML = messageTemplate.replace(':number',
				fileNumber).replace(':percents',
				Math.round(uploadProgress) + '%')
	}

	MediaManager.prototype.setUploadProgress = function(value) {
		var progressBar = this.$el.get(0).querySelector(
				'[data-control="upload-progress-bar"]')
	...
	MediaManager.prototype.updateUploadBar = function(templateName, classNames) {
		var fileNumberLabel = this.$el.get(0).querySelector(
				'[data-label="file-number-and-progress"]'), successTemplate = fileNumberLabel
				.getAttribute('data-' + templateName + '-template'), progressBar = this.$el
				.get(0).querySelector('[data-control="upload-progress-bar"]')
	...
	MediaManager.prototype.updateSearchResults = function() {
		var $searchField = this.$el.find('[data-control="search"]'), data = {
	...
	MediaManager.prototype.onSearchChanged = function(ev) {
		...
		this.searchTrackInputTimer = window.setTimeout(this
				.proxy(this.updateSearchResults), 300)
	}
	...
	MediaManager.prototype.deleteItems = function() {
		...
		$.oc.confirm(this.options.deleteConfirm, this
				.proxy(this.deleteConfirmation))
	}
	...
	MediaManager.prototype.createFolder = function(ev) {
		$(ev.target).popup(
				{
					content : this.$el.find(
							'[data-control="new-folder-template"]').html(),
					zIndex : 1200
				// Media Manager can be opened in a popup, so this new popup
	...
	MediaManager.prototype.moveItems = function(ev) {
		...
		$(ev.target).popup({
			...
			extraData : data,
			zIndex : 1200
		// Media Manager can be opened in a popup, so this new popup should have
		// a higher z-index
		})
	}
	...
	MediaManager.prototype.onMoveItemsSubmit = function(ev) {
		var items = this.$el.get(0).querySelectorAll(
				'[data-type="media-item"].selected'), data = {
	...
	MediaManager.prototype.onListMouseDown = function(ev) {
		this.itemListElement.addEventListener('mousemove', this
				.proxy(this.onListMouseMove))
		...
		this.itemListPosition = $.oc.foundation.element.absolutePosition(
				this.itemListElement, true)

		var pagePosition = $.oc.foundation.event.pageCoordinates(ev), relativePosition = this
				.getRelativePosition(this.itemListElement, pagePosition.x,
						pagePosition.y, this.itemListPosition)
	...
	MediaManager.prototype.onListMouseUp = function(ev) {
		...
		if (this.selectionStarted) {
			...
			var items = this.itemListElement
					.querySelectorAll('[data-type="media-item"]:not([data-root])'), selectionPosition = $.oc.foundation.element
					.absolutePosition(this.selectionMarker, true)

			for (var index = 0, len = items.length; index < len; index++) {
				var item = items[index], itemPosition = $.oc.foundation.element
						.absolutePosition(item, true)

				if (this.doObjectsCollide(selectionPosition.top,
						selectionPosition.left,
						this.selectionMarker.offsetWidth,
						this.selectionMarker.offsetHeight, itemPosition.top,
						itemPosition.left, item.offsetWidth, item.offsetHeight)) {
					if (!ev.shiftKey)
	...
	MediaManager.prototype.onListMouseMove = function(ev) {
		...
		var deltaX = relativePosition.x - this.selectionStartPoint.x, deltaY = relativePosition.y
		...
		if (this.selectionStarted) {
			if (deltaX >= 0) {
				this.selectionMarker.style.left = this.selectionStartPoint.x
						+ 'px'
			...
			if (deltaY >= 0) {
				...
				this.selectionMarker.style.top = this.selectionStartPoint.y
						+ 'px'
			} else {
	...
	$.fn.mediaManager = function(option) {
		...
		this.each(function() {
			...
			var options = $.extend({}, MediaManager.DEFAULTS, $this.data(),
					typeof option == 'object' && option)
			if (!data)
				$this.data('oc.mediaManager', (data = new MediaManager(this,
						options)))
