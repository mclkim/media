...
class MediaLibrary {
	 ...
	 * 
	 * @param array $exclude
	 *        	A list of folders to exclude from the result list/
	...
	public static function validatePath($path, $normalizeOnly = false) {
		...
		if (strpos ( $path, '..' ) !== false)
			throw new ApplicationException ( Lang::get ( 'cms::lang.media.invalid_path', [ 
					'path' => $path 
			] ) );
	 ...
	 * 
	 * @param string $path
	 *        	Specifies the item path relative to the storage disk root.
	 * @param string $type
	...
	protected function initLibraryItem($path, $itemType) {
		 ...
		 */
		$lastModified = $itemType == MediaLibraryItem::TYPE_FILE ? $this->getStorageDisk ()->lastModified ( $path ) : null;
		 ...
		 */
		$size = $itemType == MediaLibraryItem::TYPE_FILE ? $this->getStorageDisk ()->size ( $path ) : $this->getFolderItemCount ( $path );
	...
	protected function getFolderItemCount($path) {
		$folderItems = array_merge ( $this->getStorageDisk ()->files ( $path ), $this->getStorageDisk ()->directories ( $path ) );
	...
	protected function getStorageDisk() {
		...
		return $this->storageDisk = Storage::disk ( Config::get ( 'cms.storage.media.disk', 'local' ) );
	}
